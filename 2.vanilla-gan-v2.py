...
class GAN:
	...
	def __init__(self, sess):
		...
		with tf.name_scope("result_from_Discriminator"):
			#학습데이터가 진짜일 확률
			self.D_X, self.D_X_logits = self.Discriminator(self.X) #batch_size, 1
			#노이즈로부터 생성된 데이터가 진짜일 확률 
			self.D_Gen, self.D_Gen_logits = self.Discriminator(self.Gen, True) #batch_size, 1
...
def train(model, data):
	...
	for i in range( iteration ):
		...
		#Discriminator 학습.
		_, D_value = sess.run([model.D_minimize, model.D_loss], {model.X:input_, model.noise_source:noise})

		#Generator 학습.
		_, G_value = sess.run([model.G_minimize, model.G_loss], {model.noise_source:noise})
